...
class DoctorDetailFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (this.activity as AppActivity).supportActionBar?.subtitle = "Informacion sobre el Dr. ${args.doctor.name}"
        ...
        binding.tvDoctorDetailWorkingTime.text =
            TimeUtils.getElapsedTimeInHumanReadableFormatFromTimestamp(args.doctor.enter_date!!.toLong())
        binding.ivDoctorDetailLogo.setImageResource(IconUtils.getIdIcon(args.icon))
        val active: Boolean = args.doctor.active ?: false

        if (active) {
            binding.tvDoctorDetailState.setCompoundDrawablesWithIntrinsicBounds(
                R.drawable.circle_success,
                0,
                0,
                0
            )
        ...
        binding.ivDoctorDetailPhoto.setImageResource(
            PhotoUtils.getNamePhoto(
                args.doctor.name ?: " "
            )
        )
